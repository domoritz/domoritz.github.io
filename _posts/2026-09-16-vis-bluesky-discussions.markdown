...
Likes are also how the discussion can help with the Q&A problem from the beginning of this post. VIS used to collect questions on Slido, where attendees upvote the ones they want asked. The discussion can take the place of Slido: replies can be sorted by most liked as well as by newest, and a session chair can pick the top questions from the thread. A fun little easter egg we added is that the like button is a donut which also appears in the VIS 2026 logo. This kind of whimsical detail is only possible because we render the discussion ourselves on the website.
