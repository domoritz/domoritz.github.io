...
**Running infrastructure is a big lift for conferences.** Running infrastructure is a big burden for conferences so we simplified the setup for example by having a [webiste without dynamic content](https://github.com/ieee-vgtc/ieeevis.org). To show dynamic comments, we need to host the comments on a different server. But running and maintaining the infrastructure is more work than we want to take on so using an existing system like Bluesky and AT Proto makes sense.

**The website is static and built before any post exists.** A page cannot know the URL of its own post. So each page only has the paper id, and a small service resolves that id to the announcement when the page loads. Nothing needs a rebuild when a thread appears, moves, or is deleted. After the conference, the plan is to bake the mapping into the pages and let them read Bluesky directly, so that the service can be switched off. The discussions then depend on nothing the conference runs, and there is nothing to maintain.

**Nobody should need a Bluesky account, but a thread should still feel like a conversation between people.** Attendees without an account comment through the conference login. The comments are posted to Bluesky by a bot account. Attendee's comments show their real name by default. Attendees can hide their real name behind a pseudonym but the organizers always know who wrote what.
...
The **read proxy** relays the Bluesky API to requests to the VIS website. When a page loads, it asks for the thread that belongs to its paper id. The proxy keeps the mapping from paper id to announcement, fetches the thread from Bluesky, caches it for a few seconds, and returns it in a shape the page understands. Before the bot has posted, the page says that the discussion opens shortly before the session.

The **guest bridge** takes comments and likes from attendees without a Bluesky account. An attendee with a Bluesky account can log in with it on the page and post from their own account (the login asks only for permission to post and like, nothing else), or reply in the Bluesky app, which on a phone is nicer than anything the page could offer. Posting from their personal account is the preferred path, since the reply belongs to its author, carries their name and profile, and reaches their followers. When an attendee logs in this way, the site offers to save their handle to their conference profile, so that later announcements can mention them. An attendee without an account signs in with their conference account instead (we require login so that only attendees can use the guest posting bridge). The bridge then posts the comment from the discussion account, signed with the attendee's name, or with a pseudonym if they tick "Hide my name" for that comment. Either way it is an ordinary reply on Bluesky, and both kinds appear in the same thread.

Likes need one extra step. A shared Bluesky account can like a post only once, so guest likes cannot be stores on Bluesky. The bridge stores them itself, one per attendee per reply, and the read proxy adds them to the count from Bluesky, so a reply shows a single number wherever its likes came from.

Likes are also how the discussion can help with the Q&A problem from the beginning of this post. VIS used to collect questions on Slido, where attendees upvote the ones they want asked. The discussion can take the place of Slido: replies can be sorted by most liked as well as by newest, and a session chair can pick the top questions from the thread. A fun little easter egg we added is that the like button is a donut (VIS is in Boston). This kind of whimsical detail is only possible because we render the discussion ourselves on the website.
...
<figure>
<img src="{{ '/images/posts/vis-bluesky/architecture-after.svg' | relative_url }}" alt="Diagram of the setup after the conference: the paper page now knows the URL of its post and reads the thread from Bluesky directly, attendees with a Bluesky account still log in on the page and reply from their own account, and the discussion service in the middle is switched off.">
...
VIS is trying this for the first time in 2026, and we hope for adoption of the idea but we cannot guarantee it. Will people ask questions on the paper pages, will authors answer, and will threads keep going after the conference? People are [glad to see VIS on Bluesky](https://bsky.app/profile/chezvoila.com/post/3mvnakgshxk2d) already, which is a start. I will share what I learn and hope we continue this experiment in future years. Also, nothing here is specific to VIS. Any conference with a program database and a website can announce its papers and embed the threads, and I am happy to help others try.

Lastly, building this system was a team effort. The general chairs backed the experiment, the tech team opened their server and program database to it, and the web team reviewed and merged the changes to the site. Thanks everyone who is supporting this experiment and I hope it leads to more persistent and engaging discussions at VIS and beyond.
